...
| **CDCS (CAD)** | CAD 유전·염증 (LPA·PCSK9·LDLR·IL6·CRP) | LPA·PCSK9·IL6·CRP 부재; LDLR·MIA3 음성 | **DIVERGENT** | ⚠ 검증 CAD 축 미재현 — 신호가 비특이 전신억제일 가능성 |
...
### 4.2 CDCS / 관상동맥질환 — 검증 CAD 축 미재현 ⚠
**검증 출처:** CAD 유전(LPA, PCSK9, LDLR, SORT1) + 염증(IL6, CRP) — GWAS·임상 다중 검증.
**cfRNA 결과:** LPA·PCSK9·IL6·CRP·APOB 부재; LDLR·MIA3는 음성. 양성 신호 11개뿐, 852/861이 음성(전신 억제).
**판정:** **DIVERGENT.** CDCS의 cfRNA 신호는 **CAD-특이 지질/염증 생물학이 아니라 비특이적 전신 번역·OXPHOS 억제**로 보임.
**critical-thinking:** 만성 질환의 일반적 "전신 생물학적 기능 억제"가 CAD signature를 압도 → **질병 특이성 낮음**. 진단 marker로 부적합.
...
- **지질/염증 CAD 축** — LPA/PCSK9/IL6 → 신호가 비특이 전신억제
...
1. **번역억제 배경의 교란.** 다수 질병에서 NES<0 leading-edge가 거대(예: CDCS 852개, ME/CFS 169개). 검증 driver가 이 배경에 우연히 포함되면 "재현"으로 오인될 수 있어, 본 보고서는 **NES>0만 재현으로 인정**했다. 그래도 일부 양성 재현이 단일 광역 신호의 부산물일 가능성 잔존(**indirectness**).
