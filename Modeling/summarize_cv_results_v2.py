...
def main():
    ...
    df["group"] = df["stage"].where(df["stage"].fillna("") != "", df["route"])

    # ---- Stage-level summary table ----
    ...
    summary.to_csv(CV_DIR / "cv_summary_by_stage.csv", index=False)
    print("\nStage-level calibration summary:")
    print(summary.round(3).to_string(index=False))

    # ---- NZ-binned summary (within each stage, is calibration stable across NZ?) ----
    edges = sorted(set([0, config.MODELING_PARAMS_V2["nz_a_max"], 40, 70, 100, 200, 400, 700]))
    df["nz_bin"] = pd.cut(df["nz"], bins=edges, right=False)
    nz_summary = df.groupby(["group", "nz_bin"], observed=True).agg(
        n=("gene", "size"), median_w1=("w1", "median"), median_std_z=("std_z", "median"),
    ).reset_index()
    nz_summary.to_csv(CV_DIR / "cv_summary_by_stage_nz.csv", index=False)

    # ---- Figures ----
    groups = sorted(df["group"].dropna().unique())
    colors = {"pool": "#1b9e77", "nb_fixed": "#d95f02", "nbi": "#7570b3", "intercept": "#e7298a"}

    fig, axes = plt.subplots(1, 3, figsize=(15, 4))
    for grp in groups:
        sub = df[df["group"] == grp]
        c = colors.get(grp, "gray")
        axes[0].hist(sub["w1"].clip(0, 1), bins=40, alpha=0.5, color=c, label=f"{grp} (n={len(sub)})")
        axes[1].hist(sub["std_z"].clip(0, 3), bins=40, alpha=0.5, color=c, label=grp)
        axes[2].hist(sub["kurt_z"].clip(-2, 15), bins=40, alpha=0.5, color=c, label=grp)
    axes[0].set(xlabel="W1 to N(0,1)", ylabel="genes", title="Calibration (W1) by stage")
    axes[0].legend()
    ...
    axes[2].legend()
    fig.tight_layout(); fig.savefig(FIG_DIR / "cv_calibration_by_stage.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    for grp in groups:
        sub = df[df["group"] == grp].sort_values("nz")
        ax.scatter(sub["nz"], sub["w1"].clip(0, 1), s=4, alpha=0.3, color=colors.get(grp, "gray"), label=grp)
    ax.axvline(config.MODELING_PARAMS_V2["nz_a_max"], color="k", ls="--", lw=1)
    ax.set(xlabel="NZ", ylabel="W1 to N(0,1)", xscale="log", title="Calibration vs NZ across stages")
    ax.legend()
    fig.tight_layout(); fig.savefig(FIG_DIR / "cv_w1_vs_nz.png", dpi=150)
    plt.close(fig)

    print(f"\nSaved -> {CV_DIR}/cv_summary_by_stage.csv, cv_summary_by_stage_nz.csv")
    print(f"Figures -> {FIG_DIR}/cv_calibration_by_stage.png, cv_w1_vs_nz.png")
